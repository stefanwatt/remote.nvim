...
local ____exports = {}
____exports.readSshConfigFile = function(path)
    if path == nil then
        path = "~/.ssh/config"
    end
    local sshConfigFile = io.open(path, "rb")
    local ____sshConfigFile_read_result_0 = sshConfigFile
    if ____sshConfigFile_read_result_0 ~= nil then
        ____sshConfigFile_read_result_0 = ____sshConfigFile_read_result_0:read("*a")
    end
    local content = ____sshConfigFile_read_result_0
    local ____sshConfigFile_close_result_2 = sshConfigFile
    if ____sshConfigFile_close_result_2 ~= nil then
        ____sshConfigFile_close_result_2 = ____sshConfigFile_close_result_2:close()
    end
    if not content then
        error(
            __TS__New(Error, "error reading config file"),
            0
        )
    end
    return content
end
____exports.getSshHosts = function(sshConfigContent)
    local hostChunks = __TS__StringSplit(sshConfigContent, "Host ")
    return __TS__ArrayMap(
        __TS__ArrayFilter(
            hostChunks,
            function(____, chunk) return not not chunk and __TS__StringTrim(chunk) ~= "" end
        ),
        function(____, chunk)
            local lines = __TS__ArrayMap(
                __TS__StringSplit(chunk, "\n"),
                function(____, line) return __TS__StringTrim(line) end
            )
            local name = lines[1]
            local hostNameLine = __TS__ArrayFind(
                lines,
                function(____, line) return __TS__StringIncludes(line, "HostName") end
            )
            if not hostNameLine then
                error(
                    __TS__New(Error, "invalid ssh config"),
                    0
                )
            end
            local hostName = __TS__StringSplit(hostNameLine, " ")[2]
            local userLine = __TS__ArrayFind(
                lines,
                function(____, line) return __TS__StringIncludes(line, "User") end
            )
            ...
            local user = __TS__StringSplit(userLine, " ")[2]
            local port
            local portLine = __TS__ArrayFind(
                lines,
                function(____, line) return __TS__StringIncludes(line, "Port") end
            )
            if portLine then
                local ____portLine_split_result__1_4 = portLine
                if ____portLine_split_result__1_4 ~= nil then
                    ____portLine_split_result__1_4 = __TS__StringSplit(portLine, " ")[2]
                end
                port = ____portLine_split_result__1_4
            end
            return {name = name, hostName = hostName, user = user, port = port}
        end
    )
end
return ____exports
